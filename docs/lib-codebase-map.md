...
| --- | --- | --- | --- |
| `lib/capistrano.rb` | none directly | Main library entry point. Loads the Rake compatibility patch, configuration DSL, plugin extension system, and string helper. | `fix_rake_deprecated_dsl`, `configuration`, `extensions`, `ext/string` |
...
| `lib/capistrano/command.rb` | `Capistrano::Command` | Sequential remote command runner. Opens one SSH command channel at a time, handles stdout/stderr callbacks, optional pty, environment injection, placeholder replacement, and remote command errors. | `errors`, `processable`, `Configuration.default_io_proc`, SSH sessions from `connections` |
...
| `lib/capistrano/configuration/actions/file_transfer.rb` | `Capistrano::Configuration::Actions::FileTransfer` | Adds `put`, `get`, `upload`, `download`, and `transfer` DSL actions. Delegates actual work to `Capistrano::Transfer`. | `transfer`, `Connections#execute_on_servers`, `sessions`, `run` |
| `lib/capistrano/configuration/actions/inspect.rb` | `Capistrano::Configuration::Actions::Inspect` | Adds `stream` and `capture` actions for running commands and consuming combined or first-server output. | `errors`, `Invocation#invoke_command`, `sudo` |
...
| `lib/capistrano/configuration/connections.rb` | `Capistrano::Configuration::Connections`, `DefaultConnectionFactory` | Manages thread-local SSH sessions, failures, single-server connection establishment/teardown, current task server lookup, and continuing after remote errors where requested. | `ssh`, `errors`, `Servers`, `Execution#current_task` |
...
| `lib/capistrano/recipes/deploy/scm/base.rb` | `Capistrano::Deploy::SCM::Base`, `Base::LocalProxy` | Abstract SCM adapter API. Provides local-mode variable lookup, command construction, default command handling, logging, and abstract checkout/sync/export/diff/log/query hooks. | `Configuration` variables and logger; subclassed by all SCM adapters |
...
| `lib/capistrano/recipes/deploy/strategy/checkout.rb` | `Capistrano::Deploy::Strategy::Checkout` | Remote strategy that checks out the selected SCM revision directly into `release_path`. | `strategy/remote`, `SCM#checkout` |
...
| `lib/capistrano/recipes/deploy/strategy/export.rb` | `Capistrano::Deploy::Strategy::Export` | Remote strategy that exports the selected SCM revision directly into `release_path`. | `strategy/remote`, `SCM#export` |
...
| `lib/capistrano/ssh.rb` | `Capistrano::SSH`, `SSH::Server` | SSH connection helper. Applies the originating `ServerDefinition` to Net::SSH sessions and builds public-key-only connection options. | `net/ssh`, `ServerDefinition` |
...
flowchart TD
  ...
  strategy_base --> strategies["Checkout, Export, Copy, RemoteCache, UnsharedRemoteCache"]
...
| --- | --- |
| `lib/capistrano.rb` | `capistrano/fix_rake_deprecated_dsl`, `capistrano/configuration`, `capistrano/extensions`, `capistrano/ext/string` |
...
| `lib/capistrano/recipes/deploy/strategy/checkout.rb` | `capistrano/recipes/deploy/strategy/remote` |
| `lib/capistrano/recipes/deploy/strategy/export.rb` | `capistrano/recipes/deploy/strategy/remote` |
...
- `Command`, `Transfer`, and `Shell` all depend on `Processable` because they share the same Net::SSH event-loop pattern.
- `ServerDefinition` is the common server identity object. Server filtering, SSH connection factories, command placeholders, transfer logging, and error reporting all pass it around.
